...
| Que cada tema tenga su **esquema y resumen** de cierre, que el mapa y el resumen tengan la forma pactada y que su vocabulario salga en la página. No sabe si una frase dice lo mismo que la página: eso hay que leerlo | `python generadores/pon_cierre.py --mira` · `--valida cierres/<tema>.json` |
...
| Que la página **se vea bien**: imágenes que no cargan y dejan un hueco blanco, cajas que se salen a lo ancho y texto recortado por abajo. Mira cada sesión a **390 y a 1280 px**, y dispara antes el `loading="lazy"`, que si no la mitad de las imágenes no han llegado | `python generadores/comprueba_pinta.py [filtro]` |
