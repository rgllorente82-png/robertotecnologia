...
def test(idt, titulo, preguntas):
    ...
    partes = [u'      <div class="test" id="test-%s">\n        <h4>%s</h4>\n' % (idt, titulo)]
    for n, q in enumerate(preguntas):
        partes.append(u'        <div class="test-p" data-ok="%d">\n          <p>%d. %s</p>\n'
                      % (q['ok'], n + 1, q['p']))
        for i, op in enumerate(q['op']):
            partes.append(u'          <label class="test-op"><input type="radio" name="%s-%d" '
                          u'value="%d">%s</label>\n' % (idt, n, i, op))
        partes.append(u'          <div class="test-por"><b>Por qu&eacute;:</b> %s</div>\n'
                      u'        </div>\n' % q['por'])
    partes.append(u'''        <div class="test-pie">
          ...
          <span class="test-nota"></span>
        </div>
        <p class="test-aviso">Se corrige en tu propio navegador: no se env&iacute;a ni se guarda nada.
