...
ESC_ENGRANAJES = u'''
      ...
      (function(){
        ...
        function pinta(){
          ...
          if(modo === 'sin'){
            /* el tornillo, horizontal; la rueda encima, tangente a su lomo */
            var YT = 236, ALT = 22, XA = 190, XB = 430;
            var rp = radio(z2);
            var cx = 310, cy = YT - ALT - rp;
            m += rueda(cx, cy, z2, -ang/z2, 'var(--surface-2)');
            m += '<rect x="' + XA + '" y="' + (YT-ALT) + '" width="' + (XB-XA) + '" height="' + (2*ALT)
               + '" rx="' + ALT + '" fill="var(--surface-2)" stroke="var(--ink-soft)" stroke-width="2"></rect>';
            var des = (ang*26) % 26;
            for(i = 0; i < 12; i++){
              var x0 = XA + 8 + i*26 + des;
              if(x0 > XA + 6 && x0 + 16 < XB - 6){
                m += '<path d="M' + x0.toFixed(1) + ' ' + (YT-ALT+2) + ' l16 ' + (2*ALT-4)
                   + '" stroke="var(--ink-soft)" stroke-width="2"></path>';
              }
            }
            m += '<path d="M' + (XB+10) + ' ' + YT + ' h34" stroke="var(--goo-azul)" stroke-width="4"></path>';
            m += '<text x="' + (XB+16) + '" y="' + (YT-12) + '" class="rotulo-svg">entrada</text>';
            m += etiqueta(cx + rp + 82, cy - 4, z2, '1,5', 'la rueda, de salida');
            m += '<text x="20" y="26" class="rotulo-svg" style="font-size:12.5px;fill:var(--ink)">'
